...
public class ReflectionTest extends Parent<String> implements Serializable {
    ...
    private void typeTokenTest() {
        System.out
                .println("typeToken,对class做一些封装,支持泛型----------------------------------------------------------------");
        // 通过这种写法可以获取泛型的具体类型
        getGenericityType();
        ...
        System.out.println(
                "获取所有实现的接口,包括继承的类中的实现接口:typeToken.getTypes().interfaces():" + typeToken.getTypes().interfaces());
        System.out.println(
                "classes+interfaces很常用这个api:typeToken.getTypes().rawTypes():" + typeToken.getTypes().rawTypes());
    ...
    private void invokableTest() throws NoSuchMethodException {
        System.out
                .println("invokableTest,对Method级别的封装----------------------------------------------------------------");
